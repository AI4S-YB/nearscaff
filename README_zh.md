...
- **lr 模式（HiFi），~200 Mb 植物基因组**：1,001 个可填 gap 闭合
  75–104 个，写入 ~0.9 Mb 真实序列（填充中位 7 kb）；唯一性闸门
  拦下了 46/64 个重复驱动的假闭合候选。
- **end-join**（重叠末端直接对接，不引入新序列）：碎片化组装上
  产量最高的机制——~560 Mb 碎片化植物基因组 2,852 个对接、
  ~460 Mb 植物基因组 360 个（是其序列闭合数的 11 倍）。
- **sr 模式**（~460 Mb / ~560 Mb 植物基因组）：序列闭合 97 / 257
  个，另有 PE 缩放 267 / 585 个 gap——假阳性提醒见上文。
...
**gapfill（DNA fill）短读模式，16 线程**——两个实测植物基因组
（一个 ~460 Mb、16,649 gaps；一个碎片化 ~560 Mb、约 55 万条
contig）：
...
| **SR 模式全程（两个植物基因组）** | **105 / 190 min** | ≤6 GB（bloom） |

**rna-fill，24 线程**——我们实测的最大规模（某 ~200 Mb 高度碎片
化植物基因组：281k contig 组装、290,735 个 gap、2.5 万条转录本、
120–190 Mb 参考，`--ref` 放置 + fill-check）：
...
基于参考坐标的组件排序与定向置信度（碎片不再保持原始图顺序，而是
对参考二次重排）：

- scaffold 组件按精确化后的参考中点坐标排序；没有参考坐标的
  contig 保持图顺序、插值在相邻锚点之间（替代旧的整 scaffold
  翻转归一化）。
- 每条 scaffold 的方向归一化为参考坐标递增；每个组件按最优比对
  （最长块优先，其次 mapq）定向；未比对上的 contig 回退到遍历
  推断；仅有低置信比对（mapq < 10）支持的定向在 AGP 中记为 `?`。
- tiered PAF 输出真实链向，不再硬编码 `+`。

这一问题是在一个实测植物基因组上发现并修复的（染色体部分反向）：
与参考的定向一致率从 50%（相当于随机）提升到两个基准植物基因组
的 96.6% / 94.1%，染色体级 scaffold BUSCO 从 85.0% → 92.1%
（案例 1）和 97.4% → 97.6%（案例 2）。
