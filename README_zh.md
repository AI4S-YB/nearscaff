...
**rna-fill，24 线程**——我们实测的最大规模（某高度碎片化植物基因
组：281k contig 组装、290,735 个 gap、2.5 万条转录本、120–190 Mb
参考，`--ref` 放置 + fill-check）：
...
在一个高度碎片化的实测植物基因组上验证（28 万条 contig，用同种
的染色体级参考重挂载）：95–96% 的短填充和约 95% 的放置基因
